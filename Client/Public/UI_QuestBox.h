...
/* 체력 프레임 */
class CUI_QuestBox final : public CUI_Text
{
private:
	CUI_QuestBox(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wstring& strPrototypeTag);
	CUI_QuestBox(const CUI_QuestBox& rhs);
	virtual ~CUI_QuestBox() = default;
...
public:
	static CUI_QuestBox* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wstring& strPrototypeTag); //! 원형객체 생성
