...
PS_OUT PS_MAIN_EFFECT(PS_IN In, uniform bool bSolid)
{
    ...
    //float2 vMaskUV = In.vTexcoord * g_UVScale_Mask + g_UVOffset_Mask;
    //if (false == g_bUV_Wave)
    //{
...
PS_OUT PS_MAIN_DISTORTION(PS_IN_DISTORTION In, uniform bool bSolid)
{
    ...
    vAlphaColor = g_MaskTexture.Sample(ClampSampler, vDistortedCoord.xy + vMaskUV);
    vFinalDiffuse.a *= vAlphaColor;
    ...
    if (vFinalDiffuse.a <= g_fAlpha_Discard) // 알파 잘라내기
     ...
     // 색상 혼합
    Out.vColor.rgb = Calculation_ColorBlend(vFinalDiffuse, g_vColor_Mul, g_iColorMode).rgb;
    Out.vColor.a = vFinalDiffuse.a * g_vColor_Mul.a;
