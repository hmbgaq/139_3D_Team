			...
			if (is_selected)
			{
				...
				if (m_bCreateEffect)
				{
					...
					if (m_pBones.size() > 0)// 본이 존재한다면 
					{
						...
						EFFECT_MANAGER->Play_Effect("", m_vecEffectName[n] + ".json", nullptr, m_EffectPosition);
						//pEffect->Set_Position(m_EffectPosition);
						m_bCreateEffect = false;
					}
					else // 만약 본이 선택되지 않고 없다라면 0값으로 초기화
					{
						EFFECT_MANAGER->Play_Effect("", m_vecEffectName[n] + ".json", nullptr, _float3(0.0f, 0.0f, 0.0f));
