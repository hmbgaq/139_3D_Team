...
void CVampireCommander_Weapon_Hand::OnCollisionEnter(CCollider* other)
{
	...
	if (nullptr != pTarget_Character && m_bSynced == false)// 일반 타격 
	{
		...
		pTarget_Character->Set_Hitted(m_fDamage, Get_Object_Owner()->Calc_Look_Dir_XZ(vTargetPos) * -1, m_fForce, 1.f, m_eHitDirection, m_eHitPower);
		...
		EFFECT_MANAGER->Play_Effect("Hit/", parent->Get_CharcterDesc().EffectFileName + ".json", nullptr, this->Get_WorldPosition());
