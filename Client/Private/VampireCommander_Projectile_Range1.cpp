...
HRESULT CVampireCommander_Projectile_Range1::Initialize(void* pArg)
{
	...
	//m_pEffect = EFFECT_MANAGER->Create_Effect_With_Trail("Test_Impact.json", "Test_Effect_Trail.json", this);	// Test Impact
...
void CVampireCommander_Projectile_Range1::OnCollisionEnter(CCollider* other)
{
	...
	{
		pTarget_Character->Set_Hitted(m_iDamage, pTarget_Character->Calc_Look_Dir(m_pTransformCom->Get_Position()), m_fForce, 1.f, m_eHitDirection, m_eHitPower);
