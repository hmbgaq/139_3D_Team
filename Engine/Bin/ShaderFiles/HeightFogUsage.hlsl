...
float3 ApplyLight(float3 fogColor, float3 wpos, float3 g_WorldSpaceCameraPos)
{
    
    float3 camToPoint = normalize(wpos - (g_WorldSpaceCameraPos - HF_DirectionalLightVector * 100 * HF_LightFarness));
...
float3 ApplyFog(float3 color, float3 wpos, float3 g_WorldSpaceCameraPos)
{
        ...
        #if HF_LIGHT_ATTEN
            finalFogColor.rgb = ApplyLight(finalFogColor, wpos, g_WorldSpaceCameraPos);
#endif
...
float3 ApplyFogSkybox(float3 color, float3 wpos, float3 g_WorldSpaceCameraPos)
{
    if(HF_SkyFill <= 0)
    return color;
