...
HRESULT CRenderer::Render_Ice()
{
	...
	for (auto& pGameObject : m_RenderObjects[RENDER_ICE])
	{
		if (nullptr != pGameObject && true == pGameObject->Get_Enable())
			pGameObject->Render_Ice();
...
HRESULT CRenderer::Ready_DebugRender()
{
	...
	FAILED_CHECK(m_pGameInstance->Ready_RenderTarget_Debug(TEXT("Target_Ice"), (fSizeX / 2.f * 1.f), (fSizeY / 2.f * 11.f), fSizeX, fSizeY));
