...
namespace ImGui {
    ...
    RenderNeoSequencerTopBarOverlay(float zoom, float valuesWidth,uint32_t startFrame, uint32_t endFrame, uint32_t offsetFrame, const ImVec2 &cursor, const ImVec2 &size,
        ...
        if(drawFrameLines) {
            ...
            int32_t counter = 0;
            uint32_t primaryFrames = (uint32_t)pow(10, counter++);
            uint32_t secondaryFrames = (uint32_t)pow(10, counter);
            ...
            {
                primaryFrames = (uint32_t)pow(10, counter++);
                secondaryFrames = (uint32_t)pow(10, counter);
